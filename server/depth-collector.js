...
class DepthCollector {
    ...
    async _initSnapshot() {
        try {
            ...
            for (const [p, q] of data.bids) this.bids.set(parseFloat(p), parseFloat(q));
            for (const [p, q] of data.asks) this.asks.set(parseFloat(p), parseFloat(q));

            console.log(`🔥 Snapshot loaded for ${this.symbol.toUpperCase()} (ID: ${this.lastUpdateId})`);
            this.synced = true;
            ...
            console.error(`🔥 Failed to fetch snapshot for ${this.symbol}:`, err.message);
    ...
    _connectWS() {
        ...
        this.ws.on('error', (err) => console.error(`🔥 WS Error ${this.symbol}:`, err.message));
    ...
    _saveSnapshot() {
        try {
            ...
            const processSide = (map, isAsk) => {
                // Convert to array
                let levels = Array.from(map.entries())
                    .map(([p, q]) => ({ p, q }));
            ...
            // Db Insert
            db.insertSnapshots(this.symbol, [{
                time: now,
                bids,
                asks,
                maxVolume
